use super::{
    ...
    FunctionArgsAstNode, FunctionDeclarationAstNode, GroupAstNode, IdentifierAstNode, IfStatement,
    IndexAstNode, InfixOperatorAstNode, MapObjectDeclarationAstNode, NewAstNode, NullAstNode,
    NumberAstNode, OperatorAstNode, PostfixOperatorAstNode, PrefixOperatorAstNode, ReturnStatement,
    Statement, StringAstNode, ThrowStatement, TryCatchStatement, VarStatement,
    POSTFIX_PRECEDENCE_MAP, PRECENDENCE_MAP, PREFIX_PRECEDENCE_MAP,
};
...
impl Parser {
    ...
    fn parse_var_statement(&mut self) -> VarStatement {
        ...
        if self.tokens[self.cursor - 1].kind != RBRACE {
            assert(
                self.tokens[self.cursor].kind == EOF
                    || self.tokens[self.cursor].kind == SEMICOLON
    ...
    fn parse_expression_statement(&mut self) -> ExpressionStatement {
        ...
        assert(
            self.tokens[self.cursor].kind == EOF || self.tokens[self.cursor].kind == SEMICOLON,
            "Expect ';' or EOF",
        );
        self.cursor += 1;
    ...
    fn parse_class_statement(&mut self) -> ClassStatement {
        ...
        while self.tokens[self.cursor].kind != RBRACE {
            ...
            // ident() {} 为方法，其他均为字段
            if self.tokens[self.cursor + 1].kind == LPAREN {
                let start_token = self.tokens[self.cursor].clone();
                // 这里逻辑和函数解析的一致：先解析入参
                let name = self.tokens[self.cursor].value.to_string();
                self.cursor += 2;
                let mut params = vec![];
                ...
                let node = FunctionDeclarationAstNode {
                    token: start_token,
                    params,
                    body,
                };
                ...
                if name == "constructor" {
                    constructor = AstNode::FunctionDeclaration(node);
                } else {
                ...
                match op_node {
                    OperatorAstNode::InfixOperatorAstNode(mut infix) => {
                        if infix.token.kind == ASSIGN {
                            if infix.left.clone().as_ref().unwrap().to_string() == "constructor" {
                                panic!("constructor must be a method");
                            }
                            let point = AstNode::Operator(OperatorAstNode::InfixOperatorAstNode(
                                InfixOperatorAstNode {
                                    token: Token {
                                        kind: POINT,
                                        value: ".".to_string(),
    ...
    fn get_eof_or_operate_node(&self, token: &Token) -> OperatorAstNode {
        ...
        if let Some(_) = PRECENDENCE_MAP.get(&token.value) {
            return OperatorAstNode::InfixOperatorAstNode(InfixOperatorAstNode::new(token.clone()));
        }
        if let Some(_) = POSTFIX_PRECEDENCE_MAP.get(&token.value) {
