...
pub fn init(mut ctx: Context) -> Context {
    // Math库
    {
        ...
        let random = |_this: &mut Element, _vec: Vec<Rc<RefCell<Element>>>| {
            Rc::new(RefCell::new(Element::new_number(random::<f64>())))
        };
        let floor = |_this: &mut Element, vec: Vec<Rc<RefCell<Element>>>| match &*vec[0].borrow() {
            Element::Number { value, .. } => {
                Rc::new(RefCell::new(Element::new_number(value.floor())))
            }
            _ => panic_any("floor param should be a number"),
        };
        let ceil = |_this: &mut Element, vec: Vec<Rc<RefCell<Element>>>| match &*vec[0].borrow() {
            Element::Number { value, .. } => {
                Rc::new(RefCell::new(Element::new_number(value.ceil())))
            }
            _ => panic_any("ceil param should be a number"),
        };
        let abs = |_this: &mut Element, vec: Vec<Rc<RefCell<Element>>>| match &*vec[0].borrow() {
            Element::Number { value, .. } => {
                Rc::new(RefCell::new(Element::new_number(value.abs())))
            }
    ...
    // Time库
    {
        ...
        let now = |_this: &mut Element, _vec: Vec<Rc<RefCell<Element>>>| {
            Rc::new(RefCell::new(Element::new_number(
                Utc::now().timestamp_millis() as f64,
            )))
        };
        let sleep = |_this: &mut Element, vec: Vec<Rc<RefCell<Element>>>| {
            match &*vec[0].borrow() {
                Element::Number { value, .. } => {
    ...
    // Json库
    {
        ...
        let stringify = |_this: &mut Element, vec: Vec<Rc<RefCell<Element>>>| {
            Rc::new(RefCell::new(Element::new_string(element_to_json(
                vec[0].clone(),
            ))))
        };
        let parse = |_this: &mut Element, vec: Vec<Rc<RefCell<Element>>>| {
    ...
    // File库
    {
        ...
        let read_file = |_this: &mut Element, vec: Vec<Rc<RefCell<Element>>>| {
        ...
        let write_file = |_this: &mut Element, vec: Vec<Rc<RefCell<Element>>>| {
        ...
        let append_file = |_this: &mut Element, vec: Vec<Rc<RefCell<Element>>>| {
    ...
    // Http库
    {
        ...
        let request = |_this: &mut Element, vec: Vec<Rc<RefCell<Element>>>| {
            let method =
                Method::from_bytes(vec[0].borrow().to_string().trim().to_uppercase().as_bytes());
            if method.is_err() {
            ...
            let mut request_builder: RequestBuilder = client.request(method, url);
            ...
            match result {
                Ok(result) => {
                    ...
                    res.set(
                        "body",
                        Rc::new(RefCell::new(Element::new_string(txt.unwrap()))),
                    );
                    res.set("status", Rc::new(RefCell::new(Element::new_number(status))));
                    Rc::new(RefCell::new(res))
                }
                Err(_) => panic_any(SimpleError::new("request failed")),
            }
        };
        http_methods.insert(
            "request".to_string(),
...
fn element_to_json(ele: Rc<RefCell<Element>>) -> String {
    match &*ele.borrow() {
        ...
        Element::Array { array, .. } => {
            let content = array
                .iter()
                .map(|item| element_to_json(item.clone()))
                .collect::<Vec<String>>()
                .join(", ");
            format!("[ {} ]", content)
        }
